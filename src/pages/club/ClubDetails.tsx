...
export default function ClubDetails() {
  ...
  const findHistoric = () => {
    if (
      ...
      Object.keys(userPurchases.hook.data).length > 0
    ) {
        ...
        buttons: [
          ...
          },
          {
            ...
            handler: () => {
              ...
              fetch(
                `${process.env.REACT_APP_ENVIRONMENT === 'production' ? process.env.REACT_APP_BACKEND + '/payments/pay' : process.env.REACT_APP_BACKEND_DEV + '/payments/pay'}`,
                {
                  method: 'POST',
                  headers: {
    ...
    <IonContent>
      ...
      >
        <div className="flex flex-col h-full w-full items-center justify-center px-8 sm:px-12 divide-y gap-4">
          <p className="text-white text-[2.5rem] leading-[2.5rem] font-bold text-start w-full">
            {currentEvent.title}
          </p>
          <div
      ...
      <div className="flex flex-col px-8 sm:px-12 py-8 gap-6">
        <div className="flex flex-col">
          <p className="text-primary-700 font-bold text-[0.85rem] md:text-[1.5rem]">
            Atrações:
          </p>
          <div
            className="text-[0.85rem] md:text-[1.5rem] text-primary-700"
            dangerouslySetInnerHTML={{
        ...
        <div className="flex flex-col">
          ...
          <p className="text-[0.85rem] font-bold md:text-[1.5rem]">
            {currentEvent.address} número {currentEvent.number}, <br />
            {currentEvent.neighborhood} - {currentEvent.city} -{' '}
            {currentEvent.state}
          </p>
        </div>

        <div className="flex flex-col items-end justify-center md:mb-4">
          <p className="text-primary-700 font-bold text-[0.75rem] md:text-[1.5rem]">
            Investimento:
          </p>
        ...
        <div className="flex items-center justify-between">
          <div className="border border-primary-700 rounded-[0.625rem]">
            <button
              ...
              className=" text-primary-700 py-2 px-4 md:py-4 font-bold text-[0.75rem] md:text-[1.5rem]"
              type="button"
            >
              Voltar
            </button>
            ...
            (findHistoric() ? (
              <ModalProsper.Modal>
                <ModalProsper.ModalTrigger>
                  {' '}
                  <button className="disabled:opacity-50 py-2 md:py-4 px-4 bg-primary-700 text-white font-bold text-[0.75rem] md:text-[1.5rem] rounded-[0.625rem]">
              ...
              <button
                disabled={currentEvent.price && balance < currentEvent.price}
                className="disabled:opacity-50 py-2 px-4 md:py-4 bg-primary-700 text-white font-bold text-[0.75rem] md:text-[1.5rem] rounded-[0.625rem]"
                onClick={action}
                type="button"
              >
                {currentEvent.price ? 'Comprar Entrada' : 'Participar'}
              </button>
            ))}
        </div>

        <div className="bg-primary-700 rounded-[0.625rem] text-white p-4 mt-8 md:p-6">
          <p className="text-[0.75rem] md:text-[1.5rem] font-bold text-center">
            Nunca pediremos suas senhas pessoais ou entraremos em contato por
            meios não oficiais.
          </p>
        </div>
      </div>
          ...
          <ModalProsper.ModalContent className="w-[80%] p-0 rounded-[0.625rem] gap-0">
            <ModalProsper.ModalHeader className="bg-primary rounded-t-[0.625rem]">
              <div className="flex flex-col px-2 py-4 md:px-4 md:py-8">
                <span className="text-white text-[1.5rem] md:text-[2.5rem] font-bold">
                  Atenção!
                </span>
                <span className="text-white text-[0.75rem] md:text-[1rem]">
                  Você não tem saldo suficiente para adquirir o ingresso para
                  este evento.
                </span>
              </div>
            </ModalProsper.ModalHeader>
