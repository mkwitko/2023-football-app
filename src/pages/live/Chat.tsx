...
const Comment = ({ comment, style, index }: {
  comment: any,
  style: any
  index: number
}) => {
  return (
    <div style={style} className={`flex items-start justify-between gap-4 w-full p-4 pb-0 index-${index + 1}`}>
      ...
      <div className='flex flex-col items-start w-full'>
        ...
        <p className='w-full rounded-[0.625rem]'>{StringCutter(comment.comment, 100, '...')}</p>
      </div>
    </div>
  )
}

const SuperComment = ({ comment, style }: {
  comment: any,
  style: any
}) => {
  return (
    <div style={style}>
      <div className='flex items-start justify-between gap-4 w-full p-4 pb-2 bg-bgClub bg-opacity-20 bg-center bg-125% bg-blend-soft-light'>
        ...
        <div className='flex flex-col items-start w-full'>
          ...
          <p className='text-white w-full rounded-[0.625rem]'>{StringCutter(comment.comment, 100, '...')}</p>
        </div>
      </div>
    </div>
  )
